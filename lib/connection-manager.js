...
class ConnectionManager {
  ...
  async execute(operationFn) {
    ...
    const operationTimeoutMs = Math.max(0, Number(this.config.operationTimeoutMs || 0));

    try {
      return await withTimeout(
        operationFn(client),
        operationTimeoutMs,
        `SurrealDB operation timed out after ${operationTimeoutMs}ms`
      );
    } catch (err) {
  ...
  async _validateClient(client) {
    const timeoutMs = Math.max(
      1,
      Number(this.config.healthCheckTimeoutMs || this.config.retryDelayMs || 1000)
    );
    try {
      if (typeof client.ping === "function") {
        await withTimeout(
          client.ping(),
          timeoutMs,
          `SurrealDB health check timed out after ${timeoutMs}ms`
        );
      } else {
        await withTimeout(
          client.query("RETURN true;"),
          timeoutMs,
          `SurrealDB health check timed out after ${timeoutMs}ms`
        );
      }
  ...
  _startHealthChecks() {
    ...
    this.healthTimer = setInterval(async () => {
      ...
      try {
        ...
        const healthy = await this._validateClient(client);
        if (!healthy) {
          recycled = true;
          await this.pool.destroyClient(client);
  ...
  async _activateLiveSubscription(subscription) {
    ...
    try {
      ...
      // Prefer SDK-native live API for better compatibility across SurrealDB versions.
      if (typeof client.live === "function") {
        const liveResult = await client.live(subscription.table);
        liveId = extractLiveId(liveResult);
      }

      // Fallback for clients/environments where live() is unavailable or unexpected.
      if (!liveId) {
        const fallbackResult = await client.query(`LIVE SELECT * FROM ${subscription.table};`, {});
        liveId = extractLiveId(fallbackResult);
      }
      if (!liveId) {
...
function extractLiveId(result) {
  ...
  if (Array.isArray(first.result) && typeof first.result[0] === "string") {
  ...
  if (typeof first.uuid === "string") {
    return first.uuid;
  }
  if (typeof first.live === "string") {
    return first.live;
  }
  const coerced = coerceLiveId(first);
  if (coerced) {
    return coerced;
  }
  const fromResult = coerceLiveId(result);
  if (fromResult) {
    return fromResult;
  }
  return null;
...
function coerceLiveId(value) {
  if (value === null || value === undefined) {
    return null;
  }
  if (Array.isArray(value) && value.length === 1) {
    return coerceLiveId(value[0]);
  }
